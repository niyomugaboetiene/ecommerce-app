    ...
    useEffect(() => {
            try {
                 const GetNewProducts = async() => {
                      const product = await axios.get('http://localhost:5000/product/health', {withCredentials: true});
...
useEffect(() => {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-6">
            ...
            <h2 className="text-3xl font-bold mb-8 text-green-500">Health & Beauty Products</h2>
            {cartMessage && (
             ...
             {(products.length === 0) && (
                  <p className="text-center mt-10 text-gray-500">No Health & Beauty Products found.</p>
             )
         }
        </div>
    )
